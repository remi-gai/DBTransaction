const Database = require('./solution1');
...
describe('Database Transaction tests', () => {
  ...
  test('it should add key to deletedKeys hashmap for future deletion with commit', () => {
    mockDB.begin();
    mockDB.set('city','life');
    expect(mockDB.get('city')).toBe('life');
    mockDB.delete('city');
    expect(mockDB.deletedKeys['city']).toBe(true);
  });
  ...
  test('it should throw an error when committing one transaction', () => {
    expect(()=> {mockDB.commit()}).toThrow('Cannot commit - all the transactions are up to date');
  });

  test('it should clear the deletedKeys hashmap and decrease beginCount by 1', () => {
    mockDB.begin();
    mockDB.set('city','life');
    mockDB.delete('city');
    expect(Object.keys(mockDB.deletedKeys).length).toBe(1);
    mockDB.commit();
    expect(Object.keys(mockDB.deletedKeys).length).toBe(0);
    expect(mockDB.beginCount).toBe(0);
  });
